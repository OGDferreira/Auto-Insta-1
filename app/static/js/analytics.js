...
export function initAnalyticsModule() {
  ...
  if (!section) return;
  const refresh = async () => {
    const accountId = document.getElementById("analytics-account-filter").value;
    const accountQuery = accountId ? `&account_ids=${encodeURIComponent(accountId)}` : "";
    const response = await fetch(`/api/analytics?period_days=${document.getElementById("analytics-period").value}${accountQuery}`, { cache: "no-store" });
    ...
    document.getElementById("analytics-account-metrics").innerHTML = accounts.map(item => `
      <section class="panel analytics-account-panel">
        <div class="panel-header"><div><div class="eyebrow">Conta</div><h3>@${escapeHtml(item.account)}</h3></div><span class="section-note">${Number(item.media_count || 0).toLocaleString("pt-BR")} mídias</span></div>
        <div class="analytics-cards">
          <div class="analytics-card"><small>Seguidores</small><strong>${Number(item.followers || 0).toLocaleString("pt-BR")}</strong></div>
          <div class="analytics-card"><small>Alcance</small><strong>${Number(item.reach || 0).toLocaleString("pt-BR")}</strong></div>
          <div class="analytics-card"><small>Impressões</small><strong>${Number(item.impressions || 0).toLocaleString("pt-BR")}</strong></div>
          <div class="analytics-card"><small>Visitas ao perfil</small><strong>${Number(item.profile_views || 0).toLocaleString("pt-BR")}</strong></div>
        </div>
      </section>`).join("") || `<div class="panel"><p>Nenhuma conta disponível para o período selecionado.</p></div>`;
  ...
  document.getElementById("analytics-account-filter").addEventListener("change", refresh);
  refresh();
}
